...
NetworkTables.initialize(server='10.70.54.2') 
datatable = NetworkTables.getTable('limelight')
W = 640
H = 480
...
while True:
    ...
    if min_box is not None:
        ...
        theta = np.arctan(D/F) 
        rotation = np.rad2deg(np.arctan(D/F)) 
        driveDistance = np.sqrt(D**2 + F**2)

        datatable.putNumber('tx', driveDistance)

        datatable.putNumber('ty', theta)

        datatable.putNumber('ta', rotation)
